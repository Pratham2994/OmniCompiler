...
def predict_breakpoints(cpp_path: Path):
    ...
    if k == 0:
        print(f"\nNo confident breakpoints found for {cpp_path.name}.\n")
        return

    df_top = df_sorted.head(k)

    print(f"\nAuto-breakpoints for {cpp_path.name} (k={k}):\n")
